...
# Set the FLOCK_ID variable to the desired flock if the default one won't be used
FLOCK_ID="default"
...
curl -XGET "https://$CANARY_HASH.canary.tools/api/v1/flocks/summary" \
  ...
  -G | jq '.flocks_summary[] | .flock_id'

# List pending bird commissions after the AWS EC2 instance has been deployed and set the NODE_ID accordingly
NODE_ID=$(curl "https://$CANARY_HASH.canary.tools/api/v1/devices/commission/pending" -d auth_token="$CANARY_API_KEY" -G | jq '.devices[] | .id')
